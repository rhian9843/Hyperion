...
- [x] Fix cross-connection `next_key` allocation race on macOS — `flock(LOCK_EX)` is process-granular on macOS; two `Database` instances in the same process opening the same file both obtained LOCK_EX concurrently, both read `next_key=1`, and one got `UniqueConstraintError('Duplicate key 1')`; fixed by adding a per-file-path `threading.Lock` (`_FILE_WRITE_LOCKS` registry in `pager.py`) acquired in `Pager.begin()` and released in `commit()`/`rollback()`, giving real intra-process write serialization independent of OS flock semantics
...
- [x] Physical replication — binary-level WAL streaming from primary to replica with auto-sync every 500ms; auto-reconnect on connection loss; replica runs in read-only mode (any write raises `ReadOnlyError`); `SHOW MASTER STATUS`, `SHOW SLAVE STATUS`, `SHOW BINLOG`, `START SLAVE`, `STOP SLAVE`; replica can be promoted to primary on failure
...
- [x] Query result cache — LRU cache of recent `SELECT` results with a configurable TTL; cache key is the normalised SQL + params; invalidated on any write to a referenced table; `SHOW CACHE STATUS`, `SET CACHE ON|OFF`
...
- [x] Full column statistics with MCVs — extend `ANALYZE` to collect per-column `min_val`, `max_val`, `null_count`, and top-10 Most Common Values (MCVs) with frequencies; implement `estimateSelectivity(tbl, col, op, val)` that checks MCVs first and falls back to `1/ndv`; implement multiplicative `estimateRowCount(tbl, conditions)` over all WHERE conditions; replace the optimizer's current row-count-only cost model with selectivity-aware cardinality estimates
- [x] Column histograms — collect an equi-depth histogram (10 buckets) per column during `ANALYZE`; implement `estimateSelectivity(op, val)` for range predicates (`<`, `<=`, `>`, `>=`, `=`, `BETWEEN`); integrate histogram estimates into `estimateSelectivity()` as the fallback when a column has no MCV match; requires full column statistics as a prerequisite
- [x] DP join order planner — replace the greedy join reorderer with a bitmask DP algorithm over all 2^n subsets for globally optimal join ordering of 3+ tables; cost model: `SEQ_PAGE_COST=1.0`, `HASH_BUILD=1.5`, `HASH_PROBE=1.0`, `SORT_FACTOR=1.2`, `CPU_COST=0.01`; plan cache keyed on sorted table names to avoid re-planning the same join set; global stats tracking (`queries_planned`, `plan_cache_hits`, `plan_cache_misses`, `subsets_evaluated`); `SHOW DP STATS` command; requires full column statistics for cost model inputs
- [x] Extended selectivity operators — add stats-backed selectivity estimates for `!=`/`<>` (`1 - eq_sel`), `LIKE` (fixed 0.05 heuristic), `IS NULL` (`null_count / row_count`), and `IS NOT NULL` (`1 - null_count / row_count`); currently all four fall through to the 0.33 no-stats default in `estimate_selectivity()`
- [x] `CREATE STATISTICS` — named multi-column statistics objects (`CREATE STATISTICS name ON table(col1, col2)`) tracking cross-column correlations (dependencies, ndistinct, multi-column MCVs); prevents selectivity underestimation when correlated columns appear together in WHERE; `DROP STATISTICS`, `SHOW STATISTICS`
