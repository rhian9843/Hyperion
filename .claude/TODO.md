...
- [ ] Extended selectivity operators — add stats-backed selectivity estimates for `!=`/`<>` (`1 - eq_sel`), `LIKE` (fixed 0.05 heuristic), `IS NULL` (`null_count / row_count`), and `IS NOT NULL` (`1 - null_count / row_count`); currently all four fall through to the 0.33 no-stats default in `estimate_selectivity()`
- [ ] `CREATE STATISTICS` — named multi-column statistics objects (`CREATE STATISTICS name ON table(col1, col2)`) tracking cross-column correlations (dependencies, ndistinct, multi-column MCVs); prevents selectivity underestimation when correlated columns appear together in WHERE; `DROP STATISTICS`, `SHOW STATISTICS`
